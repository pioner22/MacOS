...
names = ['common.sh', 'count_stream.pl', 'fixtures.txt', 'metal_vram.m', 'net.sh',
         'profile.sh', 'ram_native.c', 'run.sh', 'storage_file.c', 'supervise.pl', 'report.sh', 'profiles.tsv', 'recovery.sh', 'recovery_ram.pl', 'recovery_file.pl', 'readonly.sh', 'storage_readonly.pl', 'registry.sh', 'registry_devices.tsv', 'registry_profiles.tsv', 'registry_tools.tsv', 'registry_tests.tsv']
rows = []
for name in names:
