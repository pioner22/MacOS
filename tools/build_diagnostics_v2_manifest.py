...
names = ['common.sh', 'count_stream.pl', 'fixtures.txt', 'metal_vram.m', 'net.sh',
         'profile.sh', 'ram_native.c', 'run.sh', 'storage_file.c', 'supervise.pl', 'report.sh', 'profiles.tsv', 'recovery.sh', 'recovery_ram.pl', 'recovery_file.pl', 'storage_readonly.sh', 'storage_readonly.pl']
rows = []
for name in names:
