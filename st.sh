...
macdiag_launch(){
  ...
  while IFS=$'\t' read -r sha bytes name extra;do
    case "$name" in common.sh|count_stream.pl|fixtures.txt|metal_vram.m|net.sh|profile.sh|ram_native.c|run.sh|storage_file.c|report.sh|supervise.pl|profiles.tsv|recovery.sh|recovery_ram.pl|recovery_file.pl|storage_readonly.sh|storage_readonly.pl) ;;*)boot_fail MANIFEST_PATH_INVALID 'Неизвестное имя файла в манифесте.' 'Unapproved file name in manifest.';return 3;;esac
  ...
  for name in common.sh profile.sh net.sh report.sh recovery.sh storage_readonly.sh run.sh;do /bin/bash -n "$work/$name" || { boot_fail PACKAGE_SYNTAX_FAILED "Ошибка синтаксиса: $name." "Syntax error: $name.";return 3; };done
