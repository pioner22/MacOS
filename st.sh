...
macdiag_launch(){
  ...
  while IFS=$'\t' read -r sha bytes name extra;do
    case "$name" in common.sh|count_stream.pl|fixtures.txt|metal_vram.m|net.sh|profile.sh|ram_native.c|run.sh|storage_file.c|report.sh|supervise.pl|profiles.tsv|recovery.sh|recovery_ram.pl|recovery_file.pl|readonly.sh|storage_readonly.pl|registry.sh|registry_devices.tsv|registry_profiles.tsv|registry_tools.tsv|registry_tests.tsv) ;;*)boot_fail MANIFEST_PATH_INVALID 'Неизвестное имя файла в манифесте.' 'Unapproved file name in manifest.';return 3;;esac
  ...
  [ "$n" -eq 22 ] || { boot_fail PACKAGE_INCOMPLETE 'Пакет неполный; запуск запрещён.' 'Incomplete package; execution blocked.';return 3; }
  for name in common.sh profile.sh net.sh report.sh recovery.sh readonly.sh registry.sh run.sh;do /bin/bash -n "$work/$name" || { boot_fail PACKAGE_SYNTAX_FAILED "Ошибка синтаксиса: $name." "Syntax error: $name.";return 3; };done
